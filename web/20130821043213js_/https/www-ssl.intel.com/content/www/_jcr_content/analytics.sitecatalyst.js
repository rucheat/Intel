...
function record(event, values, collect) {
if (collect) {
return [event, values];
} else if (record.callbacks) {
if (event) {
//record data to clickStreamCloud
CQ_Analytics.EventDataMgr.reset();

if (typeof event == "string") {
CQ_Analytics.EventDataMgr.setProperty("events", event);
} else {
CQ_Analytics.EventDataMgr.setProperty("events", event.join("\u2026"));
}
if (values) {
for (var j in values) {
CQ_Analytics.EventDataMgr.setProperty(j, values[j]);
}
}
...
//run callbacks for omniture tracking
for (var i = record.callbacks.length - 1; i >= 0; i--) {
if (record.callbacks[i].call(this, event, values)) {
//clear events cache
...
record.callbacks = [];
record.callbacks.push(function(event, values) {
s = s_gi("intelreimglobal");
...
var a = CQ_Analytics.EventDataMgr.getProperty("events").split("\u2026");
var i = a.length;
while (i--) {
if (a[i] === event) {
return true;
...
if (mgr == "eventdata" && (s.linkTrackVars.indexOf(evar) < 0)) {
if (s.linkTrackVars == "None") {
...
s.visitorNamespace = "intelwap"
s.trackingServer = "www91.intel.com"
s.trackingServerSecure = "www90.intel.com"
// Custom WAP -------------------------------------------------------------------------------
...
+ "n,1,1).t()};s.wd.s_dc=function(un){s_gi(un,1).t()}}", w = window, l = w.s_c_il, n = navigator, u = n.userAgent, v = n.appVersion, e = v.indexOf('MSIE '), m = u.indexOf('Netscape6/'), a, i, x, s;
if (un) {
un = un.toLowerCase();
...
/*
     FILE ARCHIVED ON 04:32:13 Aug 21, 2013 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 16:55:45 Jun 21, 2023.
...
/*
playback timings (ms):
  captures_list: 669.786
  exclusion.robots: 0.128
  exclusion.robots.policy: 0.122
  cdx.remote: 0.045
  esindex: 0.006
  LoadShardBlock: 646.137 (3)
  PetaboxLoader3.datanode: 234.62 (4)
  PetaboxLoader3.resolve: 447.971 (2)
  load_resource: 46.901
*/